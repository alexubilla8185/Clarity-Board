...
const QuickStartGuide: React.FC = () => (
    <div className="space-y-6 text-on-surface-variant">
        <h3 className="display-small text-on-surface">Welcome to Clarity Board!</h3>
        <p className="body-large">Here’s a quick guide to get you started with the main features.</p>
        ...
        <div className="flex items-start gap-4">
            ...
            <div>
                <h4 className="title-large text-on-surface">Create Anything</h4>
                <p className="body-medium">Use the floating <span className="font-bold">+</span> button in the bottom right to add new items. You can choose from four types:</p>
                <ul className="list-disc ml-6 mt-2 space-y-1 body-medium">
        ...
        <div className="flex items-start gap-4">
            ...
            <div>
                <h4 className="title-large text-on-surface">Enhance Cards with AI</h4>
                <p className="body-medium">Supercharge individual tasks. Click the sparkles icon on any Kanban card to:</p>
                <ul className="list-disc ml-6 mt-2 space-y-1 body-medium">
        ...
        <div className="flex items-start gap-4">
            ...
            <div>
                <h4 className="title-large text-on-surface">100% Private & Offline</h4>
                <p className="body-medium">Your data is yours alone. Everything is stored locally in your browser, ensuring complete privacy and seamless offline access. No accounts, no cloud, no tracking.</p>
            </div>
        </div>
    </div>
);
        ...
        {changelogData.map(release => (
            <div key={release.version}>
                <div className="flex items-baseline gap-4 mb-3">
                    <h3 className="headline-small text-on-surface">Version {release.version}</h3>
                    <span className="body-medium text-on-surface-variant">{release.date}</span>
                </div>
                <div className="space-y-4">
                    {Object.entries(release.changes).map(([type, changes]) => (
                        <div key={type}>
                            ...
                            `}>{type}</h4>
                            <ul className="list-disc ml-6 mt-2 space-y-2 text-on-surface-variant body-medium">
        ...
        <div className="fixed inset-0 bg-scrim/60 flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true" aria-labelledby="help-title">
            <div ref={trapRef} className="bg-surface rounded-lg shadow-3 w-full max-w-3xl max-h-[90vh] flex flex-col animate-fade-in-up">
                <header className="p-4 flex justify-between items-center border-b border-outline flex-shrink-0">
                    <h2 id="help-title" className="text-on-surface title-large">Help Center</h2>
                ...
                <div className="border-b border-outline flex-shrink-0">
                    <div className="p-2 flex gap-2" role="tablist">
                        <button 
                            ...
                            className={`px-4 py-2 rounded-full label-large ${activeTab === 'guide' ? 'bg-primary/20 text-primary' : 'hover:bg-surface-container'}`}>
                            Quick Start
                        </button>
                        <button 
                            role="tab"
                            ...
                            className={`px-4 py-2 rounded-full label-large ${activeTab === 'changelog' ? 'bg-primary/20 text-primary' : 'hover:bg-surface-container'}`}>
                            Changes
                        </button>
                    </div>
                </div>
